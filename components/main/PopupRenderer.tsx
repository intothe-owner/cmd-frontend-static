...
export default function PopupRenderer() {
  ...
  useEffect(() => {
    ...
    const fetchActivePopups = async () => {
      try {
        const res = await fetch(apiUrl("/api/popups/active"));
        ...
        if (json.success) {
          ...
          const validPopups = json.data.filter((p: any) => {
            ...
            // WINDOW 타입 팝업 처리
            if (p.type === "WINDOW") {
              window.open(`/popup-view/?id=${encodeURIComponent(String(p.id))}`, `popup_${p.id}`, "width=400,height=500,left=100,top=100");
              return false;
            }
            return true;
          });
