...
export default function BoardConfigManager() {
  ...
  const fetchConfigs = async () => {
    setLoading(true);
    try {
      const res = await fetch(apiUrl("/api/board-configs"));
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const url = isEditing
        ? apiUrl(`/api/board-configs/${formData.id}`)
        : apiUrl("/api/board-configs");
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      const res = await fetch(apiUrl(`/api/board-configs/${id}`), { method: "DELETE" });
      const json = await res.json();
      if (json.success) {
        alert("삭제되었습니다.");
        fetchConfigs();
