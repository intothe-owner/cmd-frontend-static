...
export default function PopupManager() {
  ...
  const fetchPopups = async () => {
    const res = await fetch(apiUrl("/api/popups"));
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const url = isEditing ? apiUrl(`/api/popups/${formData.id}`) : apiUrl("/api/popups");
  ...
  const handleDelete = async (id: number) => {
    ...
    const res = await fetch(apiUrl(`/api/popups/${id}`), { method: "DELETE" });
