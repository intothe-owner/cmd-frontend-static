      ...
      <div className="max-w-6xl mx-auto px-4 w-full">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-end mb-8 gap-6 border-b border-slate-200 pb-6">
          ...
          <div className="flex flex-col sm:flex-row gap-3 w-full md:w-auto">
            ...
            {userLevel >= boardConfig.writeLevel && (
              <Link href={boardWriteHref(boardId)} className="bg-blue-600 text-white px-6 py-2.5 rounded-xl text-sm font-semibold hover:bg-blue-700 transition-colors shadow-sm shrink-0 flex items-center justify-center">
                글쓰기
              </Link>
            )}
          </div>
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
            ...
            <div className="hidden md:block overflow-x-auto">
              <table className="w-full text-left border-collapse">
                ...
                <tbody className="divide-y divide-slate-100">
                  {posts.map((post: any) => (
                    <tr key={post.id} className="hover:bg-slate-50/50 transition-colors group">
                      ...
                      <td className="py-4 px-6">
                        <Link href={boardPostHref(boardId, post.id)} className="flex items-center text-slate-800 group-hover:text-blue-600 font-medium transition-colors">
                          {post.isNotice && <span className="px-2 py-0.5 rounded text-xs font-bold bg-red-100 text-red-600 mr-3">공지</span>}
                          {post.title}
                        </Link>
                      </td>
                  ...
                  {posts.length === 0 && (
                    <tr><td colSpan={5} className="py-16 text-center text-slate-500">등록된 게시글이 없습니다.</td></tr>
                  )}
                </tbody>
              </table>
            </div>
            ...
            <ul className="block md:hidden divide-y divide-slate-100">
              {posts.map((post: any, index: number) => (
                <li key={post.id} ref={index === posts.length - 1 ? lastPostElementRef : null} className="p-4 hover:bg-slate-50 active:bg-slate-100 transition-colors">
                  <Link href={boardPostHref(boardId, post.id)} className="block">
              ...
              {posts.length === 0 && <li className="py-12 text-center text-slate-500">등록된 게시글이 없습니다.</li>}
            </ul>
          </div>
        )}
        ...
        {boardType === 'GALLERY' && (
          <div className="grid gap-6 grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
            {posts.map((post: any, index: number) => (
              <Link key={post.id} href={boardPostHref(boardId, post.id)} ref={index === posts.length - 1 ? lastPostElementRef : null} className="group flex flex-col bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden hover:shadow-md transition-all duration-300">
                ...
                <div className="p-5 flex-1 flex flex-col">
                  <h3 className="font-bold text-slate-800 text-lg line-clamp-1 mb-2 group-hover:text-blue-600 transition-colors">{post.title}</h3>
        ...
        <div className="hidden md:flex justify-center items-center gap-2 mt-12">
          {Array.from({ length: totalPages }, (_, i) => i + 1).map((p) => (
            <button 
              ...
              className={`w-10 h-10 rounded-xl font-medium transition-all ${page === p ? 'bg-blue-600 text-white shadow-md shadow-blue-600/20' : 'bg-white border border-slate-200 text-slate-600 hover:bg-slate-50 hover:border-slate-300'}`}
            >
              {p}
            </button>
          ))}
        ...
        <div className="md:hidden flex justify-center mt-8">
          {loading && <div className="text-sm font-medium text-slate-500 flex items-center gap-2"><span className="w-4 h-4 border-2 border-slate-300 border-t-blue-600 rounded-full animate-spin"></span>불러오는 중...</div>}
          {!loading && page >= totalPages && posts.length > 0 && <span className="text-sm font-medium text-slate-400 bg-slate-100 px-4 py-2 rounded-full">모든 글을 불러왔습니다</span>}
        </div>
      </div>
    </div>
  );
